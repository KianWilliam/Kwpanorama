...
//		$doc->addScript(\Joomla\CMS\Uri\Uri::root(true) . '/media/jui/js/jquery.min.js');
	//	$doc->addStyleSheet(Uri::Base() . 'components/com_kwpanorama/assets/panobox.css');
		    $wa->registerAndUseStyle('panoboxcss', Uri::Base() . 'components/com_kwpanorama/assets/css/panobox.css');

	//	$doc->addScript(Uri::Base() . 'componoents/com_kwpanorama/assets/panobox.js');
		    $wa->registerAndUseScript('panoboxjs', Uri::Base() . 'components/com_kwpanorama/assets/js/panobox.js');
...
//	}
	...
	?>
		...
		<link rel="stylesheet" href="<?php echo \Joomla\CMS\Uri\Uri::root(true) ?>/components/com_carouselck/assets/font-awesome.min.css" type="text/css" />
	...
	static function resizeImage($file, $x, $y = '', $thumbpath = 'th', $thumbsuffix = '_th') {
		...
		if ($size) {
			...
			if (!\Joomla\CMS\Filesystem\Folder::exists($thumbfolder)) { 
				...
				\Joomla\CMS\Filesystem\File::copy(JPATH_ROOT . '/modules/mod_carouselck/index.html', $thumbfolder . 'index.html' );
			}

			if ($size['mime'] == 'image/jpeg') {
				$img_big = imagecreatefromjpeg($filetmp); # En ouvrent de l'image d'origine
...
//		$item->imgthumb = $item->thumb;
	...
	static function setImageUrl($url) {
		if (strpos($url, 'http') !== 0) {
			$url = \Joomla\CMS\Uri\Uri::root(true) . '/' . $url;
		}
...
*/
	public static function getProMessage() {
		$html = '<div class="kwpinfo"><i class="fas fa-info"></i><a href="https://www.kwproductions121.ir/components/kwpanorama.html" target="_blank">' . \Joomla\CMS\Language\Text::_('KWProductions Co.  Component Panorama') . '</a></div>';
...
/*
	public static function getProPreview($imgName) {
		$html = '<div class="ckpropreview">'
					...
					. '<a href="https://www.joomlack.fr/en/joomla-extensions/carousel-ck" target="_blank">' . \Joomla\CMS\Language\Text::_('CAROUSELCK_ONLY_PRO') . '</a>'
				. '</div>';
