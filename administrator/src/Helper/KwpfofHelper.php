...
use Joomla\Component\Kwpanorama\Administrator\Helper\KwpinputHelper;
use Joomla\Component\Kwpanorama\Administrator\Helper\KwptextHelper;
...
class KwpfofHelper {
	...
	static protected $environment = 'com_kwpanorama'; // for joomla only

	static protected $input;

	public static function loadHelper($name) {
		require_once(JPATH_SITE . '/administrator/components/com_kwpanorama/helper/Kwp' . $name . '.php');
	}
	...
	public static function _die($msg = '') {
		$msg = $msg ? $msg : Text::_('JERROR_ALERTNOAUTHOR');
		jexit($msg);
	}
	...
	public static function getToken($name = '') {
		return Factory::getSession()->getFormToken() . '=1';
	}
...
//			, $db->quoteName('profile_key') . ' = ' . $db->quote('custom.%')
	...
	public static function addToolbarTitle($title, $image = '') {
		ToolbarHelper::title($title, $image);
	}
	...
	public static function addToolbarPreferences() {
		...
		if (self::userCan('admin')) {
			ToolbarHelper::preferences(self::$environment);
		}
	}
	...
	public static function getModel($modelName, $classPrefix = 'Kwpanorama') {
			return KWPModel::getInstance($modelName, $classPrefix);
	}
}
